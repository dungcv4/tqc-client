// Source: Ghidra work/06_ghidra/decompiled_full/WndDragBeginMethod/ (4 .c)
// String literal #3460 = "BtnDragBegin_CallBack"
...
public class WndDragBeginMethod : IWndComponent, IBeginDragHandler, IEventSystemHandler
{
	...
	public override void InitComponent(WndForm wnd)
	{
		if (wnd != null && !string.IsNullOrEmpty(_methodName))
		{
			...
			else
			{
				...
				_method = wndType.GetMethod("BtnDragBegin_CallBack", types);
				if (_method != null)
				{
					_wnd = wnd;
	...
	// Source: Ghidra OnBeginDrag.c RVA 0x1958450
	// 1-1:
	//   if WaitQuitApp() || _wnd == null || !_wnd.IsActive() return;
	//   isLuaWnd = (_wnd.GetType() == typeof(WndForm_Lua))
	//   if _method == null return;
	//   if (!isLuaWnd) _methodParams[1] = eventData
	//   else _methodParams[2] = eventData
